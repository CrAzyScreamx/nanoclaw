...
interface PusherConfig {
  ...
  secret: string;
  intervalMs?: number;
  /** Address the dashboard is bound to; where snapshots are POSTed. */
  host?: string;
}

/** Every IP currently assigned to a local interface (incl. Docker bridges). */
...
export async function startDashboard(): Promise<void> {
  const env = readEnvFile(['DASHBOARD_SECRET', 'DASHBOARD_PORT', 'DASHBOARD_HOST']);
  ...
  const host = process.env.DASHBOARD_HOST || env.DASHBOARD_HOST || '0.0.0.0';
  if (!secret) {
    log.info('Dashboard disabled (no DASHBOARD_SECRET)');
    return;
  }
  // The bind address is usually a Docker bridge gateway (reachable only from
  // containers on that network, never from the LAN). That address disappears
  // if the network is removed, and listen() would then throw EADDRNOTAVAIL as
  // an unhandled 'error' event — taking the whole host process down. Skip the
  // dashboard instead: it is optional, the host is not.
  if (host !== '0.0.0.0' && !localAddresses().includes(host)) {
    log.warn('Dashboard disabled (DASHBOARD_HOST not a local address)', { host });
    return;
  }
  const { startDashboard: startServer } = await import('@nanoco/nanoclaw-dashboard');
  startServer({ port, secret, host });
  startDashboardPusher({ port, secret, host, intervalMs: 60000 });
}
...
function postJson(config: PusherConfig, urlPath: string, data: unknown): void {
  ...
  const req = http.request({
    hostname: config.host && config.host !== '0.0.0.0' ? config.host : '127.0.0.1',
    port: config.port,
    path: urlPath,
    method: 'POST',
    headers: {
